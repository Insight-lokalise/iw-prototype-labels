...
export const ProductSEO = ({reviews}) => {
    ...
    const { product, specifications } = useContext(PDPContext)
    const materialId = product?.materialId;
    const availabilityLabel  = (product?.availability?.availabilityMessage === "inStock"
                                || product?.availability?.unlimited) ? "InStock" : "OutOfStock";
    const itemConditionMap = {open: "UsedCondition", rf: "RefurbishedCondition", ref: "RefurbishedCondition"};
    const itemIdSuffix = (/-(ref|rf|open)$/i.exec(materialId) || ["",""])[1].toLowerCase();
    ...
    const specValue = function(specName, label) {
        return specifications?.filter(s=>s.label===specName)[0]?.details.filter(d=>d.label===label)[0]?.value;
    };
    ...
    return (
        <div className="seo-tags">
            <meta itemProp="name" content={seoEncode(product?.descriptions?.shortDescription)} />
            <meta itemProp="description" content={seoEncode(product?.descriptions?.longDescription)} />
            ...
            {product?.price?.listPrice && (
                <span itemProp="offers" itemScope itemType={`${schemaOrgSite}/Offer`}>
                    <meta itemProp="price" content={product?.price?.listPrice} />
                    <meta itemProp="priceCurrency" content={product?.price?.currency} />
                    <link itemProp="availability" href={`${schemaOrgSite}/${availabilityLabel}`} />
                    <link itemProp="itemCondition" href={`${schemaOrgSite}/${itemCondition}`} />
                </span>
            )}
            {reviews?.totalReviews && (
              <div itemProp="aggregateRating" itemScope itemType={`${schemaOrgSite}/AggregateRating`}>
