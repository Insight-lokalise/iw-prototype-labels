...
export default function PasswordExpiredForm(props) {
  ...
  return (
    <Form
      onSubmit={onSubmit}
      validate={validatePasswordExpiredForm}
      skipValidateOnMount
      render={({handleSubmit}) => {
        return (
          <form onSubmit={handleSubmit} className="c-login-form" aria-labelledby="loginHeading">
            <h1 id="loginHeading" className="u-h2">{t("Password expired")}</h1>
            <p>{t('Your password has expired. Enter a new password below.')}</p>
