...
export function loadUser() {
    return window.InsightUserObject.ready()
        .then(user => {
            return {
                ...
                requiresTermsAcceptance: user.requiresTermsAcceptance,
                sessionId: user.sessionId,
                termsId: user.termsId,
                userInformation: user.userInformation,
                userPermissions: user.userPermissions,
                webGroupPermissions: user.webGroupPermissions,
                consortiaAgreements: user.consortiaAgreements
            }
        })
}
...
export function getDefaultCarrier() {
    return post('transaction/getFreightCostByCarrier', '')
        .catch((error) => {
...
export function getDefaultBillingAddress() {
    return get('transaction/getDefaultBillingAddress')
    .catch((error) => {
...
export function getDefaultShippingAddress() {
    return get('transaction/getDefaultShippingAddress')
    .catch((error) => {
...
/**
 * fetch new company standards name
 */
export function getNewStandardsName(webGroupId) {
  return get(`/insightweb/hasNewCStandards/${webGroupId}`)
    .catch((error) => {
      console.warn('Failed to new company standards name', error)
      throw error
    })
}
