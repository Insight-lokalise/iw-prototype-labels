...
import { IWTooltipModal, IWAnchor } from '../../../iw-components'
import SummaryRow from './SummaryRow'
import Currency from '@insight/toolkit-react/lib/Currency/Currency'
import { t } from '@insight/toolkit-utils/lib/labels'

export default function EWRFee(props) {
    const ewrText = 'EWR'
    const ewrTitle = 'Electronic waste recycling'
    const modalBody = props.isCanada ? <CanadaEWRFeeModalBody locale={props.locale} /> : <CaliforniaEWRFeeModalBody />
    return (
        <SummaryRow
            className="cart-summary__ewr"
            title={
                <span>
                    {t(ewrText)}
                    <IWTooltipModal
            ...
            priceElement={<Currency value={props.ewrFee} currencyCode={props.currency} />}
        />
    )
}
...
function CaliforniaEWRFeeModalBody() {
...
function CanadaEWRFeeModalBody(props) {
    const paragraphOne = (
      <p>
        {t(
