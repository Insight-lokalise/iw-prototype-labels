...
import { getProductDetailsCachedData } from './getProductDetails'


/** Get Third party warranties API
 *
 * Get third party warranties using the provided options
 ...
 */
export const getThirdPartyWarranties = async (materialId, salesOrg) => {
  const {
    userInformation,
    isIpsLogo: ipsUser,
    isLoggedIn,
  ...
  try {
    ...
    const locale = l()
    const { product } = await getProductDetailsCachedData({
      materialId,
      locale,
      salesOrg,
    })
    ...
    const request = {
      ces: true, //app-product-details is always CES, including default logged-out users
      contractId: '',
      categoryCode,
      materialQualifier,
      matId: materialId,
